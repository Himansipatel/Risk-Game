...
public class AggressivePlayer extends AbstractPlayer {
	...
	public GamePlay reinforce(GamePlay game_play) {
		...
		if (game_play != null) {
			...
			game_manager.calculateArmiesReinforce(game_play.getGame_state(), game_play.getMap(), game_play.getCurrent_player());
			...
			if (strongest_territory!=null) {
				strongest_territory.setNumber_of_armies(strongest_territory.getNumber_of_armies()+current_player.getArmy_stock());
					...
					if (defender_territory.getName().equalsIgnoreCase(strongest_territory.getTerritory_name()) 
						...
						if (defender_player!=null) {
							...
							if (game_play.getStatus().contains("Attacker Occupies Defender Territory")) {
								...
								current_player.getTerritory_list().set(current_player.getTerritory_list().indexOf(strongest_territory), strongest_territory);
								current_player.getTerritory_list().set(current_player.getTerritory_list().indexOf(defender_territory_data), defender_territory_data);
							}else {
								break;
							}
						}
		...
		if (current_player!=null) {
			...
			for (GamePlayTerritory territory_a : current_player.getTerritory_list()) {
				...
				for (GamePlayTerritory territory_b : current_player.getTerritory_list()) {
					...
					if (territory_a.getTerritory_name().equalsIgnoreCase(territory_b.getTerritory_name()) || territory_b.getNumber_of_armies() == 1) {
						continue;
					}else {
						neighbour_flag = checkIfNeighbours(territory_a.getTerritory_name(),territory_b.getTerritory_name(),game_play.getGui_map());
					}

					if (neighbour_flag==true) {
						territory_a.setNumber_of_armies(territory_a.getNumber_of_armies() + territory_b.getNumber_of_armies() - 1);
