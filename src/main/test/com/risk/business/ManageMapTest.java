...
public class ManageMapTest {
	...
	public void testMapDiscontinuity() {
		...
		map_model 							= manageMap.getFullMap("Switzerland - Invalid - Disconnected Geneva.map");		
		...
		message           	= "";
		map_model       	= new com.risk.model.Map();
		map_model 			= manageMap.getFullMap("USA - Invalid Disconnected 3 States.map");		
		message             = manageMap.checkDiscontinuity(map_model);
		assertEquals(message, "Disconnected Territories:Oklahoma-Colorado-New Mexico");

		//Valid Map
		message           	= "";
		map_model       	= new com.risk.model.Map();
		map_model 			= manageMap.getFullMap("Asia.map");		
		message             = manageMap.checkDiscontinuity(map_model);
	...
	public void testCheckInvalidNeighbour() {
		...
		String message           			= "";
		map_model       					= new com.risk.model.Map();
		map_model 							= manageMap.getFullMap("Asia - Invalid - NonExistantCountry - Yemen.map");		
		message             				= manageMap.checkInvalidNeighbour(map_model);
	...
	public void testCheckDuplicateTerritory() {
		...
		//Some Territory repeated twice in .
		com.risk.model.Map       map_model  = new com.risk.model.Map();
		map_model 							= manageMap.getFullMap("World (small) - Invalid-DuplicateTerritory.map");		
